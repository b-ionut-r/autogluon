...
class AbstractTabularTrainer(AbstractTrainer[AbstractModel]):
    ...
    def _train_multi_initial(
        ...
        if feature_prune_kwargs is not None and len(models) > 0:
            ...
            if bagged:
                bagged_model_fit_kwargs = self._get_bagged_model_fit_kwargs(
                    k_fold=k_fold, k_fold_start=k_fold_start, k_fold_end=k_fold, n_repeats=n_repeats, n_repeat_start=0, level=kwargs.get("level", 1)
                )
                model_fit_kwargs.update(bagged_model_fit_kwargs)
    ...
    def _get_bagged_model_fit_kwargs(self, k_fold: int, k_fold_start: int, k_fold_end: int, n_repeats: int, n_repeat_start: int, level: int = 1) -> dict:
        ...
        # Add cv_feature_generator ONLY for level 1 (base models)
        # Level 2+ stackers receive stacked features (original + OOF predictions) which the
        # generator may not handle correctly
